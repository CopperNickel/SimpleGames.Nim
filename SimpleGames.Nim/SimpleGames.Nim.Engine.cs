...
namespace SimpleGames.Nim {
  ...
  /// Nim Game
  /// </summary>
  /// <seealso cref="https://en.wikipedia.org/wiki/Nim"/>
  //
  //-------------------------------------------------------------------------------------------------------------------
  ...
  public sealed class NimGame : IEquatable<NimGame> {
    ...
    private long Adjust(int i) {
      ...
      return current < 0 ? -1 : saved - current;
    }
    ...
    /// <summary>
    /// Hint (best move if position is won)
    /// </summary>
    public static (int heap, long count, bool win) HintMove(IEnumerable<long> heaps) {
    ...
    /// ToString
    /// </summary>
    public override string ToString() => string.Join(" ", m_Heaps.OrderBy(item => item));

    /// <summary>
    /// Boolean - IsWinning
    /// </summary>
    public static implicit operator bool(NimGame value) =>
      (value is not null) && value.IsWin; 
    ...
    public override int GetHashCode() =>
      unchecked((int)(m_Heaps.Count ^ (m_Max << 4)));
